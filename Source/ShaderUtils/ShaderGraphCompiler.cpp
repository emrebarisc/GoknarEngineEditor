...
void ShaderGraphCompiler::CompileMaterial(const ShaderGraphCompileInput& input, MaterialInitializationData* outMaterialData) const
{
	...
	{
		inputPinToOutputPin[link.endPinId] = link.startPinId;
		++outputPinUseCount[link.startPinId];
	}
	...
	StageResult vertexStage = CompileStage({ "World Position Offset" });
	...
	StageResult fragmentStage = CompileStage({ "Base Color", "Emissive", "Normal", "Ambient Occlusion", "Metallic", "Roughness" });
	...
	auto AssignMaterialResult = [&](const std::string& masterPinName, ShaderFunctionAndResult& targetResult)
	{
	...
	AssignMaterialResult("Normal", outMaterialData->fragmentNormal);
	...
	const auto vertexValueIterator = vertexStage.masterValues.find("World Position Offset");
	if (vertexValueIterator != vertexStage.masterValues.end())
	{
		const auto masterPinIterator = std::find_if(master->inputs.begin(), master->inputs.end(), [](const ShaderPin& pin)
		{
			return pin.name == "World Position Offset";
		});
		if (masterPinIterator != master->inputs.end())
		{
			outMaterialData->vertexPositionOffset.result = FormatOutputValue(vertexValueIterator->second.expression, vertexValueIterator->second.type, masterPinIterator->type) + ";";
		}
	}
